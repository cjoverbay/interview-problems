...
def solution_reverse_words(list_of_chars, delimiter=' '):
    """
    ...
    for i in range(len(list_of_chars)):
        if i == len(list_of_chars) - 1 or list_of_chars[i+1] == delimiter:
            # reverse each word
            p_02_ReverseStrings.solution_reverse(list_of_chars, word_start, i)
            word_start = i + 2

    # reverse whole string
    return p_02_ReverseStrings.solution_reverse(list_of_chars)
